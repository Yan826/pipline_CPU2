...
//-----{指令译码}begin
    ...
    //新添加缺少的的指令
    wire inst_ADD,inst_SUB,inst_ADDI;
    wire inst_DIV,inst_DIVU;
    wire op_zero;  // 操作码全0
    wire sa_zero;  // sa域全0
    assign op_zero = ~(|op);
    assign sa_zero = ~(|sa);
    assign inst_DIV = op_zero & (rd==5'd0) & sa_zero &(funct == 6'b011010);//有符号除法
    ...
    assign inst_ADDIU = (op == 6'b001000);             //立即数有符号加法
    assign inst_ADDI  = (op == 6'b001001);             //立即数无符号加法
    ...
    assign inst_add = inst_ADDU | inst_ADDIU | inst_ADD|inst_ADDI | inst_load
    ...
    assign inst_no_rt = inst_ADDIU | inst_SLTI | inst_SLTIU
                      ...
                      | inst_J     | inst_JAL  | inst_MFC0|inst_ADDI
                      | inst_SYSCALL;
//-----{指令译码}end
...
//-----{ID->EXE总线}begin
    ...
    wire divider1,divider2;
    wire mthi;             //MTHI
    wire mtlo;             //MTLO
    assign multiply = inst_MULT;
    assign mthi     = inst_MTHI;
    assign mtlo     = inst_MTLO;
    assign divider1 = inst_DIV;
    assign divider2 = inst_DIVU;
    //ALU两个源操作数和控制信号
